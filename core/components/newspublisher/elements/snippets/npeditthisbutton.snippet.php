...
$showdeleteCaption = $defaultDeleteCaption = !empty($showdeleteCaption) ? $showdeleteCaption : $modx->lexicon('np_delete');
...
if ($showdelete && !$modx->hasPermission('delete_document')) {
      $showdeleteCaption = $modx->lexicon('np_no_context_delete_document_permission');
}
...
$showdelete = $showdelete && ($showdeleteCaption == $defaultDeleteCaption || $debug);
...
if ($showdelete) {
    $output .= "\n" . '<input type="submit" class="delete" name="delete" value="' . $showdeleteCaption . '" onclick="return confirm(' . "'" . $modx->lexicon('np_confirm_delete') . "'" . ');" />';
